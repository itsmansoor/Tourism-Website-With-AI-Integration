...
const Booking = () => {
  ...
  useEffect(() => {
    ...
    setCurrentDate(date);
  }, [params?.packageId, getPackageData]);

  // UPDATE BOOKING DATA
  useEffect(() => {
    if (packageData && params?.packageId && currentUser) {
      setBookingData((prev) => ({
        ...prev,
        packageDetails: params.packageId,
        buyer: currentUser._id,
  ...
  const handleBookPackage = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:8000/api/booking/book-package/${params?.packageId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="w-full flex flex-col items-center">
      <div className="w-[95%] rounded-lg flex flex-col items-center p-6 shadow-md gap-6">
        ...
        <div className="w-full flex flex-wrap justify-center gap-4">
          {/* USER INFO */}
          <div className="pr-3 md:border-r md:pr-6">
            <div className="w-full flex items-center justify-center bg-[#EB662B] rounded-lg">
              <div className="w-full text-white flex flex-col gap-6 p-6 rounded-md">
                <div className="flex flex-col gap-4 w-full">
                  <div>
          ...
          <div className="pl-3 md:border-l md:pl-6">
            <div className="flex flex-col gap-3">
              ...
              <p className="text-xl font-semibold text-center">
                Total Price:
                <span className="text-[#EB662B] ml-2">
                  $
                  {packageData.packageDiscountPrice
                    ? packageData.packageDiscountPrice *
                      bookingData.persons
              ...
              <div className="my-2 max-w-[300px]">
                ...
                <button
                  className="p-2 mt-2 rounded bg-[#EB662B] text-white hover:opacity-95 w-full"
                  onClick={handleBookPackage}
                  disabled={loading || !currentUser?.address}
                >
                  {loading
